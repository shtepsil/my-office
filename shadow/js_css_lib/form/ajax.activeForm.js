...
(function ($) {
                ...
                $form.ajaxForm(
                    {
                        ...
                        success: function (data_return) {
                            if (typeof data_return.errors != 'undefined') {
                                ...
                                $.growl.error({title: 'Ошибка', message: 'Ошибка в заполнении документа!', duration: 5000});
                                ...
                                if (typeof data_return.message != 'undefined') {
                                    ...
                                    if (typeof message.success != 'undefined') {
                                        ...
                                        $('input[type=file].form-control', $form).each(function () {
                                            $(this).replaceWith($(this).clone(true));
                                        })
                                    }
                                }
                                if (typeof data_return.set_value != 'undefined') {
                                    $.each(data_return.set_value, function (input, val) {
                                        $('input[name=' + input + ']', $form).val(val);
                                    });
                                }
                                if (typeof data_return.url != 'undefined') {
                                    var isUrl = (window.location.pathname + window.location.search)==data_return.url;
                        ...
                        error: function (e) {
                            if (e.status==403) {
                                $.growl.error({title: 'Ошибка', message: "У вас нету прав на данное действие!", duration: 5000});
                            } else {
    ...
    var updateInputs = function ($form, messages, submitting) {
        ...
        if (submitting) {
            ...
            $.each(data.attributes, function () {
                if (!this.cancelled && (this.status === 2 || this.status === 3)|| data.ajax_update) {
                    updateInput($form, this, messages);
                }
            });
            if (data.ajax_update) {
        ...
        if (messages) {
            ...
            $.each(data.attributes, function () {
                if ($.isArray(messages[this.id]) && messages[this.id].length) {
                    ...
                    if(summary_errors==''){
                        summary_errors=$('<ul style="padding-left: 0px;"></ul>')
                    }
                    summary_errors.append(error);
                }
            });
            if (summary_errors!='') {
                $.growl.error({title: 'Ошибка', message: summary_errors.get(0).outerHTML, duration: 5000});
            }
