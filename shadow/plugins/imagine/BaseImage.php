...
namespace shadow\plugins\imagine;
...
class BaseImage
{
    ...
    public static function text($filename, $text, $fontFile, array $start = [0, 0], array $fontOptions = [])
    {
        ...
        $img = static::getImagine()->open(Yii::getAlias($filename));
        $palette = new RGB();
        $color = $palette->color('#'.$fontColor, 100);
    ...
    public static function resize($filename, $width, $height, $mode = ImageInterface::THUMBNAIL_INSET)
    {
        $box = new Box($width, $height);
        $img = static::getImagine()->open(Yii::getAlias($filename));
    ...
    public static function thumbnailPropByHeight($filename,$height, $mode = ManipulatorInterface::THUMBNAIL_OUTBOUND)
    {
        $img = static::getImagine()->open(Yii::getAlias($filename));
        $sizeH = $img->getSize()->getHeight();
        $sizeW = $img->getSize()->getWidth();
        $prop = $height/$sizeH;
        $sizeWi = $sizeW*$prop;
        $box = new Box($sizeWi,$height);
        $thumb = $img->thumbnail($box, $mode);
        return $thumb;
    }
    public static function thumbnailPropByWidth($filename,$width, $mode = ManipulatorInterface::THUMBNAIL_OUTBOUND)
    {
        $img = static::getImagine()->open(Yii::getAlias($filename));
        $sizeH = $img->getSize()->getHeight();
        $sizeW = $img->getSize()->getWidth();
        $prop = $width/$sizeW;
        $sizeWi = $sizeH*$prop;
        $box = new Box($width,$sizeWi);
        $thumb = $img->thumbnail($box, $mode);
        return $thumb;
    }

    /**
     * Сбор информации об изображении и о файле изображения
     * @param $filename
     * @param bool $host
     * @return array
     */
    public static function getInstance($filename,$host=false){
        $img = [];
        ...
        if($host !== false){
            $filename = Yii::$app->request->hostInfo.'/'.$filename;
        }

        // Информация по изображению
        $img_obj = static::getImagine()->open(Yii::getAlias($filename));

        // Информация по файлу
        $f_info = new \SplFileInfo($filename);
        $f_info->getSize();

        $img['file_size'] = $f_info->getSize();
        $img['path'] = $f_info->getPath();
        $img['path_name'] = $f_info->getPathname();
        $img['base_name'] = $f_info->getBasename();
        $img['name'] = $f_info->getFilename();
        $img['ext'] = $f_info->getExtension();
        $img['width'] = $img_obj->getSize()->getWidth();
        $img['height'] = $img_obj->getSize()->getHeight();
