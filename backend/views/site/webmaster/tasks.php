...
<div class="wrap tss" data-project-id="<?= Yii::$app->request->get('id') ?>">
    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            ...
            <table class="table head-info" data-url="ajax/create-report" data-type-method="post">
                <thead>
                    <th>Проект</th>
                    <th class="no-paid">
                        Сумма закрытых задач<br>
                        <span class="notices">Закрытые задачи, которые попадут в отчёт</span>
                    </th>
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="list-group-item-warning t-nopaid" data-wh-ids='<?= $total['no_paid']['wh_ids'] ?>'
                            ...
                            <button <? ($total['no_paid']['cost'] == '0') ? 'disabled' : '' ?>
                                name="total_payment" data-type="create_report" class="btn btn-primary btn-sm">
                                Создать отчёт</button>
                        </td>
<!--                        <td class="list-group-item-primary t-waitpay" data-wh-ids="--><?php //= $total['waitpay']['wh_ids'] ?><!--"-->
<!--                            data-p-ids="--><?php //= $total['waitpay']['p_ids'] ?><!--">-->
<!--                            <span data-cost="--><?php //= $total['waitpay']['cost'] ?><!--">-->
<!--                                --><?php //= number_format($total['waitpay']['cost'], 2, '.', ' ') ?><!--</span>-->
<!--                            <button --><?php //= ($total['waitpay']['p_ids'] == '' and $total['waitpay']['wh_ids'] == '') ? 'disabled' : '' ?>
...
/**
    ...
    $('.tss [name=total_payment]').on('click', function () {
        var tthis = $(this),
            ...
            request_type = tthis.attr('data-type'),
            other_button = false,
        ...
        if (request_type != 'create_report' && request_type != 'proof_payment') return;

        if (request_type == 'create_report') {
            var get_sum = nopaid,
                set_sum = waitpay,
        ...
        } else if (request_type == 'proof_payment') {
            var get_sum = waitpay,
                set_sum = paid,
                get_wh_ids = get_sum.attr('data-wh-ids'),
                set_wh_ids = set_sum.attr('data-wh-ids'),
        ...
        Data['debug'] = {
            ...
            method: table.attr('data-type-method'),
        }
